...
export class ApplicationComponent implements OnInit {
	...
	// startTimer(): void {
	// 	this.orderBuy = this.orderBuy.map((order:IOrderBuy) => { return {...order, autoCancelAfterSec: (order?.autoCancelAfterSec || 0) }})
	// 	// this.timer = time * 1000;
	// 	this.timerInterval = setInterval(() => {
	// 		if (this.timer > 0) {
	// 			// this.timer -= 1000;
	// 			this.orderBuy = this.orderBuy.map((order:IOrderBuy) => {return {...order, autoCancelAfterSec: (order?.autoCancelAfterSec || 0) - 1}})
	// 		} else {
	...
	startPolling(): void {
		this.intervalSubscription = interval(1000)
			.pipe(switchMap(() => this._orderService.getSellRequest(this.id)))
			.subscribe({
				next: (data) => {
					this.order = data;
	...
	cancel() {
		this._orderService
			.sellRequestCancel({ sellRequestId: this.id })
			.subscribe((data) => {
				this.getOrder();
			});
	}
	...
	buyAccept(id: number) {
		this._orderService
			.buyRequestAccept({ buyRequestId: id })
			.subscribe((data) => {
	...
	buyCancel(id: number) {
		this._orderService
			.buyRequestCancel({ buyRequestId: id })
			.subscribe((data) => {
				this._orderService.getSellRequest(this.id).subscribe((data) => {
					this.getOrdcerBy();
				});
	...
	buyReceiveApprove() {
		this._orderService
			.buyRequestReceiveApprove({ buyRequestId: this.buyId })
			.subscribe((data) => {
				this._orderService.getSellRequest(this.id).subscribe((data) => {
					this.getOrdcerBy();
				});
	...
	submit2() {
		this._orderService
			.sellRequestModeration({
				sellRequestId: this.id,
	...
	getBuyRequestChat() {
		this._orderService.getBuyRequestChat(this.chatId).subscribe({
			next: (response) => {
				this.chats = response;
			},
			error: (error) => console.error("Fayl yuklashda xatolik:", error),
	...
	getChat() {
		forkJoin(this.orderBuy.map(item => 
			this._orderService.getBuyRequestChat(item.id).pipe(
				map(data => ({ ...item, chats: data }))
			)
		)).subscribe(updatedOrderBuy => {
			this.orderBuy = updatedOrderBuy;
		});
	}
